...
class GitHubAuth:
    ...
    async def _oauth_flow(self) -> str:
        ...
        async def handle_callback(request):
            ...
            try:
                token = await self._exchange_code_for_token(code, client_id, client_secret)
        ...
        logger.info(f"Opening browser for GitHub authentication...")
        webbrowser.open(auth_url)
        
        try:
            # Wait for token
            token = await asyncio.wait_for(token_future, timeout=300)  # 5 minute timeout
            self.token = token
    ...
    async def _exchange_code_for_token(self, code: str, client_id: str, client_secret: str) -> str:
