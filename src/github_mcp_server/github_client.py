...
class GitHubClient:
    ...
    def list_issues(
        ...
        try:
            ...
            issues = repo.get_issues(
                state=state, labels=labels or [], sort=sort, direction=direction
            )
    ...
    def get_user_info(self, username: Optional[str] = None) -> Dict[str, Any]:
        ...
        try:
            user = (
                self.github.get_user(username) if username else self.github.get_user()
            )
            return {
                "login": user.login,
                "name": user.name,
    ...
    def update_pull_request(
        ...
        try:
            ...
            logger.error(
                f"Failed to update pull request {pr_number} in {repo_name}: {e}"
            )
            raise

    def add_comment(self, repo_name: str, number: int, comment: str) -> IssueComment:
